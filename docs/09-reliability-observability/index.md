...
- [AI 系统故障模式：GPU、NCCL、网络、存储与 Runtime](ai-failure-modes-gpu-nccl-network-storage.md)：解释如何从症状、影响范围、时间线、分层信号和 evidence pack 定位 AI 系统故障，覆盖 GPU Xid/ECC/OOM/降频、DCGM 健康门禁、NCCL timeout/hang、RDMA/RoCE 网络、checkpoint 与数据存储、Runtime 状态机、调度 placement、环境漂移、runbook、自动隔离、SLO 关联和 benchmark 沉淀。
- [Incident Response、Runbook 与故障复盘：从止血到可复现改进](incident-response-runbook-postmortem.md)：解释 AI 系统事故发生时如何宣告 incident、设置 severity、分配 commander/ops/comms/scribe 角色、维护 live state document、执行 stop-the-bleeding mitigation、设计 runbook、交接 handoff、编写 blameless postmortem，并把事故沉淀为 alert、benchmark、health check、架构改进和 AI-readable incident knowledge card。
