...
很多 Agent 负载会调用代码解释器、浏览器、文件系统、数据分析脚本或第三方 CLI。为了安全，工具执行通常不能直接跑在主服务进程里，而要放进隔离沙箱。
...
- 远程执行 worker。
- 只读文件系统加临时工作目录。
- 带网络、CPU、内存、磁盘和时间限制的执行环境。

沙箱解决安全边界，但也引入冷启动成本。一次 Agent 请求如果要启动 Python、安装依赖、挂载文件、加载浏览器、建立网络连接，端到端延迟会被显著拉长。

沙箱快启关注的就是如何减少这段启动时间。常见做法包括：

| 方案 | 含义 | 风险 |
| --- | --- | --- |
| warm pool | 预先保留一批空闲沙箱 | 资源占用上升 |
| snapshot/restore | 从初始化后的快照恢复沙箱 | 随机数、密钥、连接状态要重新唯一化 |
| image pre-pull | 提前拉取镜像和依赖层 | 镜像版本和缓存治理更复杂 |
| lazy loading | 先启动最小环境，按需加载工具 | 首次访问某工具仍可能慢 |
| dependency cache | 缓存 package、模型、小数据文件 | 需要隔离租户和版本 |
| state reset | 复用沙箱前清理进程、文件、网络和权限 | 清理不彻底会产生安全问题 |

这里的 PVM 可以按沙箱快启语境理解为 pre-warmed VM、pooled VM 或 persistent VM 一类机制：系统维护一组已经启动、已经加载基础运行时的虚拟机/微虚拟机/执行环境，请求到来时快速绑定任务，任务结束后重置或销毁。

不同系统里 PVM 可能是具体产品名、内部术语或泛称，所以落地时要写清楚它到底指什么：

- 是完整 VM、microVM、容器，还是进程池。
- 是否跨请求复用。
- 是否保存用户状态。
- 如何做租户隔离。
- 如何清理文件、环境变量、网络连接和后台进程。
- 失败或超时时如何回收。
- 快启收益和常驻资源成本如何权衡。

对 Agent 系统来说，沙箱快启不能只看启动时间。还要同时看安全、可复现和可观测性。

建议记录这些指标：

- sandbox cold start / warm start latency。
- PVM pool 命中率。
- snapshot restore latency。
- tool execution latency。
- sandbox reset latency。
- 每个沙箱的 CPU、内存、磁盘、网络限制。
- 沙箱启动失败率、超时率和强制回收次数。
- 每个请求触发多少次沙箱执行。

如果一个 Agent benchmark 只测 LLM tokens/s，不记录沙箱启动和工具执行，就会严重低估真实端到端成本。
