...
| 11 | [Prefill/Decode 分离部署](prefill-decode-disaggregation.md) | 将两类阶段放到不同资源池，理解 PDD、KV 传输以及异构下 Attention-FFN Disaggregation（AFD）的资源拆分。 |
| 12 | [MoE 模型推理优化](moe-inference-optimization.md) | 处理专家路由、负载均衡、通信和显存问题。 |
| 13 | [EP Size 与大 EP / 小 EP](ep-size-large-small-ep.md) | 理解 MoE 专家并行大小如何影响显存、通信、DP 副本和尾延迟。 |
| 14 | [单机推理服务架构](single-node-serving-architecture.md) | 梳理一台机器上的模型加载、执行、队列和 API 服务。 |
| 15 | [多机分布式推理](distributed-inference.md) | 扩展到多 GPU、多节点和跨节点并行。 |
| 16 | [缓存体系](cache-system.md) | 统一理解 query、embedding、prefix、KV、tool result 等缓存。 |
| 17 | [Benchmark 方法](benchmark-methodology.md) | 设计可复现实验，避免只看单个吞吐数字。 |
| 18 | [vLLM](vllm.md) | 作为现代开源推理引擎的主线案例。 |
| 19 | [TensorRT-LLM](tensorrt-llm.md) | 作为 NVIDIA 高性能推理栈案例。 |
| 20 | [SGLang](sglang.md) | 作为结构化生成和高性能 runtime 案例。 |
| 21 | [RAG / Agent 推理负载](rag-agent-workloads.md) | 研究复合推理链路、工具调用、代码执行沙箱、沙箱快启、PVM、缓存和端到端可靠性。 |
| 22 | [Benchmark 方法与性能剖析](benchmark-profiling.md) | 把压测、profiling 和容量分析连接起来。 |
...
详见：[Prefix Cache](prefix-cache.md)
...
Prefill/Decode 分离部署把 Prefill 和 Decode 放到不同 GPU 池或不同服务角色中，避免计算密集型 Prefill 与带宽敏感型 Decode 混在一起互相干扰。更细的异构拆分还会讨论 AFD，即 Attention-FFN Disaggregation。

详见：[Prefill/Decode 分离部署](prefill-decode-disaggregation.md)

## MoE 模型推理优化

MoE 模型每个 token 只激活部分专家，但系统上会引入专家路由、负载不均、跨卡通信和专家权重放置问题。MoE 推理优化的关键不只是算力，还包括通信和调度。

详见：[MoE 模型推理优化](moe-inference-optimization.md)

## EP Size 与大 EP / 小 EP

EP Size 决定一个 MoE 副本里的专家被切到多少张 GPU 上。大 EP / 小 EP 的取舍会影响每卡专家显存、all-to-all 通信范围、DP 副本数、Prefill/Decode 表现和尾延迟。

详见：[EP Size 与大 EP / 小 EP](ep-size-large-small-ep.md)
...
RAG / Agent 不是单次模型调用，而是检索、rerank、上下文拼接、工具调用、多轮规划和多次 LLM 调用组成的复合 workload。它会放大尾延迟、缓存复杂度和失败概率，也会引入代码执行沙箱、沙箱快启、PVM 池化和工具执行隔离问题。
...
详见：[Benchmark 方法与性能剖析](benchmark-profiling.md)
