...
推理请求不是一次简单的模型调用，而是一条端到端链路。一个在线请求通常会经过 API 接入、鉴权、排队、调度、tokenization、Prefill、Decode、streaming 输出、日志记录和指标上报。

本节后续重点回答：

- 请求从进入网关到返回最后一个 token，中间经过哪些系统组件。
- 哪些环节在 CPU 上，哪些环节在 GPU 上，哪些环节受网络影响。
- 为什么同一个模型在离线脚本里很快，上线成服务后可能变慢。
