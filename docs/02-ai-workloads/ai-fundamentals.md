...
> 如果稍微改变某个参数，loss 会变大还是变小？往哪个方向变？

这个方向信息叫梯度。严格来说，梯度指向 loss 增加最快的方向；要让 loss 变小，参数要沿梯度的反方向更新（这一点在训练章节会用更新公式写清楚）。

可以用非常简单的直觉理解：

```text
loss 告诉你错得多严重
gradient 告诉你参数与 loss 之间的变化关系
optimizer 沿着让 loss 下降的方向真正更新参数
```

如果把参数想象成旋钮：

- loss 告诉你当前声音很刺耳；
- 梯度告诉你每个旋钮往哪边拧会让声音更刺耳（也就是告诉你往哪边拧会更轻）；
- 优化器决定每次拧多大，并且朝让声音变轻的方向拧。

后面的训练章节会继续解释反向传播和优化器。

本页只需要先记住：

> 梯度是训练时用于调整参数的方向信息，优化器沿梯度的反方向更新参数来降低 loss。
