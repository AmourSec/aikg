...
分析 AI 系统通信时，不要先问“这台机器有没有某种高速链路”，而要先拆成五个问题：

| 问题 | 例子 |
| --- | --- |
| 传什么 | parameter、gradient、activation、KV Cache、expert token、checkpoint shard |
| 从哪到哪 | GPU 到 GPU、GPU 到 NIC、GPU 到 CPU、节点到节点、GPU 到 SSD |
| 什么时候传 | 每层一次、每个 micro-batch 一次、每个 step 一次、每个请求一次 |
| 走什么路径 | HBM、NoC、NVLink、PCIe、NIC、IB/RoCE、CXL、存储网络 |
| 是否阻塞关键路径 | 能 overlap、能预取、能缓存，还是必须等通信结束才能继续算 |

同样是 10 GB 数据，意义完全不同：

- 如果是训练 checkpoint，每 30 分钟异步写一次，主要是存储吞吐问题。
- 如果是 FSDP 参数 all-gather，每层 forward 前都要等，可能直接影响 step time。
- 如果是 Decode 每个 token 都要读取的 KV Cache，哪怕单次数据量不大，也会高频阻塞 TPOT。
- 如果是 MoE token dispatch，平均带宽看起来够，尾部 expert 或拥塞路径仍可能拖慢整步。
...
| 数据对象 | 典型通信 | 频率 | 性能敏感点 |
| --- | --- | --- | --- |
| gradients | AllReduce / ReduceScatter | 每个训练 step，多 bucket | overlap、bucket size、跨节点带宽 |
| parameters | AllGather / Broadcast | FSDP/ZeRO 每层或每 step | prefetch、shard placement、峰值显存 |
| activations | P2P / AllGather / ReduceScatter | 每个 micro-batch、每层或 stage | pipeline bubble、层内同步 |
| KV Cache | P2P / cache transfer | 推理请求生命周期内，高频或按段迁移 | TTFT、TPOT、长上下文带宽 |
| MoE tokens | AllToAll | 每个 MoE layer | bisection bandwidth、负载均衡、tail rank |
| optimizer state | shard sync / checkpoint | 每 step 或 checkpoint | 显存、offload、恢复时间 |
| checkpoint shard | storage write/read | 低频但数据大 | 异步、backpressure、恢复 RTO |
| dataset batch | storage/network read | 每 step 或预取 | DataLoader stall、cache hit、存储网络隔离 |

这个表的用法是：先把目标 workload 的通信对象列出来，再决定哪些对象必须放在最快链路，哪些可以通过缓存、分层、异步或 offload 处理。
...
```text
communication_time ~= startup_latency + message_bytes / effective_bandwidth
```
...
- startup latency、kernel launch、协议开销更重要。
- 很多小 collective 会浪费带宽。
- fusion、bucket、batching 可能比提高链路峰值更有效。

当 message 很大时：

- effective bandwidth 更重要。
- 链路带宽、路径拥塞、PCIe/NIC/NVSwitch 上行、bisection bandwidth 会成为主因。
- 分层 collective、多 rail、拓扑感知映射更重要。

AI workload 两种情况都会出现：

- TP 层内同步常常频率高，不能只看总数据量。
- FSDP/ZeRO bucket 太小会被 latency 主导，太大又错过 overlap 窗口。
- MoE AllToAll 既有大量消息，又有负载不均衡，容易同时遇到带宽和尾延迟问题。
- 推理 KV transfer 可能是中等大小消息，但直接影响用户请求尾延迟。

所以互连调优经常不是“把 bandwidth 调大”这么简单，而是在 message size、bucket、group、拓扑和 overlap 之间找平衡。
...
- PCIe link 是否降速或降宽，例如本应是 x16 却只训练到 x8。
- BIOS、IOMMU、ACS、Resizable BAR 等设置是否影响 P2P 或 DMA 路径。
- GPU、NIC、NVMe 是否争用同一个 PCIe switch uplink。
- 多 socket 系统中，进程 CPU affinity 是否让 GPU 访问远端 host memory。
- 容器或虚拟化层是否改变设备可见性和 NUMA 信息。

这些问题不一定会让程序报错，但会让同样代码在两台机器上性能差很多。
...
## GPU Direct RDMA：GPU 与 NIC 的直接路径

多机 AI 训练经常希望 NIC 直接读写 GPU memory，而不是让数据先拷到 CPU memory，再由 CPU 侧网络栈发送。这就是 GPU Direct RDMA 这类能力要解决的问题。
...
相比绕 CPU host memory，直接路径可以减少拷贝、减少 CPU 参与、降低延迟，并提高大消息带宽。但它依赖很多条件：

- GPU、NIC 和驱动栈支持。
- GPU 与 NIC 的 PCIe/NUMA 拓扑合理。
- RDMA memory registration 和权限正确。
- NCCL/RCCL/MPI 等通信库能识别并使用该路径。
- 容器、Kubernetes、Slurm 等运行环境没有屏蔽必要设备。
- IOMMU、ACS 等平台配置没有让 P2P 路径退化。

GPU Direct RDMA 不是一个开关打开就一定快。实际还要用 benchmark 和 profiler 证明数据没有绕回 host。

常见排查信号：

- GPU-to-NIC bandwidth 明显低于预期。
- CPU memory bandwidth 或 CPU utilization 异常升高。
- 某些 GPU rank 比其他 rank 慢，刚好对应远端 NIC。
- NCCL 日志里选择了非预期 transport。
- 跨 socket GPU/NIC 组合性能显著变差。

## InfiniBand、RoCE 和拥塞控制

InfiniBand 和 RoCE 都能承载 RDMA，但运维重点不同。

InfiniBand 更像专用高性能 fabric，网络、协议和管理栈一体化程度更高。RoCE 在 Ethernet 上承载 RDMA，生态和交换机选择更广，但要让以太网在 RDMA 场景下稳定低抖动，必须认真配置无损或近似无损网络能力。
...
- DCQCN 或厂商拥塞控制机制。
- QoS / traffic class。
- switch buffer 和 pause storm 风险。
- 多租户流量隔离。
- telemetry：packet drop、ECN mark、PFC pause、retransmit、CNP。

对 AI workload 来说，拥塞问题经常是同步放大的：很多 rank 会在接近同一时间启动 collective，流量峰值非常集中。如果网络只在平均吞吐上看起来健康，仍可能在 collective 边界产生 p99 抖动、NCCL timeout 或 step time 长尾。
...
- 不同 message size。
- 多 job 并发。
- checkpoint / storage 流量同时存在。
- p50/p95/p99 和重试/拥塞计数。
...
- 小消息 latency 通常更好。
- rank 数增加时路径长度比 ring 更有优势。
- 适合 broadcast、reduce 等模式。
...
Hierarchical collective 按硬件层次分阶段通信：
...
- 算法和调度更复杂。
- 分组、rank mapping 和 message size 设置更重要。
- 多 rail、多 NIC、异构拓扑下更难验证。

NCCL/RCCL/MPI 这类通信库会根据拓扑、message size、collective 类型和环境变量选择算法。性能排查时要看实际选择了什么算法和 transport，而不是只看 API 名字。

## NCCL / RCCL / MPI 运行层

上层框架通常不会直接操作 RDMA verbs 或每条 NVLink。它会通过通信库完成 collective。

典型路径是：

```text
PyTorch / Megatron / DeepSpeed / vLLM
  -> ProcessGroup / runtime communication API
  -> NCCL / RCCL / MPI / vendor collective library
  -> transport: NVLink / PCIe P2P / SHM / RDMA / TCP
  -> NIC / switch / GPU fabric
```

通信库的职责包括：

- 发现 GPU、NIC、PCIe、NVLink、网络拓扑。
- 选择 transport。
- 选择 ring、tree、collnet、hierarchical 等算法。
- 切分 message chunk。
- 管理通信 stream。
- 和 CUDA/ROCm runtime 协调同步。
- 处理多 NIC、多 rail、多节点路径。
...
- Process group 划分不合理。
- NCCL 拓扑识别错误。
- transport fallback 到低速路径。
- NIC rail 使用不均。
- CUDA stream 同步导致 overlap 失败。
- 网络出现拥塞或错误重传。

排查时要同时看 framework trace、NCCL/RCCL 日志、系统拓扑和网络 counters。
...
大规模 GPU 节点经常有多张 NIC 或多个端口。多 rail 的目标是让一个 job 可以同时使用多条网络路径，提高注入带宽，并降低单条链路拥塞。
...
- 每张 GPU 到哪张 NIC 最近。
- 每个 rail 是否接到独立交换路径。
- 不同 rail 是否最终汇聚到同一个瓶颈 uplink。
- 通信库是否真的把流量分摊到多 rail。
- rank mapping 是否让本地 GPU 使用本地 NIC。
- 多租户调度是否把多个重通信 job 压到同一 rail。

如果 rail 分配不合理，会出现一种典型现象：所有设备都“有网络”，但某些 rank 的 collective 总是慢，尾部 step 时间被少数链路拖住。

## 拓扑感知设计样例

假设有 16 个节点，每节点 8 张 GPU，节点内 GPU fabric 很强，节点间网络也不错但显著慢于节点内。
...
DP/FSDP = 16，跨节点扩展
PP = 视模型层数和显存压力决定
EP = 优先限制在节点内或少量相邻节点，单独验证 AllToAll
```

原因是：

- TP 每层都通信，最好留在节点内高速域。
- DP/FSDP 通信可以 bucket，并且有更多 overlap 机会。
- PP 跨节点可行，但要看 activation 大小和 pipeline bubble。
- EP 如果跨 16 节点做全局 AllToAll，可能被 bisection bandwidth 和负载不均衡拖住。
...
这样避免不必要的层内同步，用多副本提高吞吐。

这个例子不是固定方案，而是说明：并行策略要从通信频率和拓扑成本推导，而不是先拍一个并行度组合。
...
| 结果类型 | 说明 |
| --- | --- |
| raw link 能力 | 例如 P2P bandwidth、RDMA bandwidth，验证链路本身 |
| collective 能力 | 例如 AllReduce、AllToAll，验证通信库和拓扑 |
| workload 影响 | 例如 step time、TTFT、TPOT，验证是否进入关键路径 |

如果 raw link 正常、collective 异常，常见原因是拓扑、rank mapping、算法或多 rail 使用不对。

如果 collective 正常、workload 异常，常见原因是 bucket、overlap、stream 同步、计算不均衡、DataLoader、checkpoint 或 runtime 调度。

如果 raw link 就异常，先不要调模型参数，要先排查硬件链路、驱动、BIOS、NIC、switch 和 cabling。
...
互连问题的难点是：应用层看到的经常只是“慢”或“timeout”，真正原因可能在硬件、驱动、通信库、rank mapping 或 workload 调度。
...
- GPU pair 之间是 NVLink/NVSwitch、PCIe P2P 还是跨 socket。
- GPU 和 NIC 是否邻近。
- PCIe link speed / width 是否符合预期。
- NIC 端口是否协商到正确速率。
- 多 rail 是否都在线。
- 是否有 ECC、PCIe replay、link error、NVLink error。
...
- NCCL/RCCL 是否识别到预期 GPU 和 NIC。
- 是否启用了 RDMA，而不是 fallback 到 TCP 或 host staging。
- collective 使用了哪些算法和 channel。
- 是否有 NCCL timeout、retry、async error。
- rank 到 device 的绑定是否和预期一致。

很多问题出在“以为走了高速路径，实际走了低速路径”。

### 第三步：拆开 microbenchmark 和 workload

如果 workload 慢，不要直接断定网络慢。

需要分开验证：

- GPU P2P benchmark 是否正常。
- RDMA bandwidth 是否正常。
- NCCL collective benchmark 是否正常。
- 真实模型的通信时间是否暴露在 critical path。
- profiler timeline 里计算和通信是否 overlap。
...
| 层次 | 需要观察什么 |
| --- | --- |
| 应用层 | step time、TTFT、TPOT、p99、tokens/s、通信占比 |
| 框架层 | bucket、process group、overlap、CUDA stream、rank mapping |
| 通信库层 | NCCL/RCCL algorithm、transport、channel、timeout、async error |
| 系统/网络层 | GPU/NIC 拓扑、PCIe/NVLink error、RDMA counters、switch congestion、PFC/ECN |

一个成熟的集群不应该只在故障后手工跑命令，而要把这些信息纳入 benchmark report、job manifest、dashboard 和告警。
...
- GPU Direct RDMA：GPU 到 NIC 是否走 direct path，是否有 host staging。
- 多 rail：rail 是否真实分摊流量，是否存在热点 uplink。
- Rank mapping：高频通信 group 是否放在近邻。
- Collective：AllReduce、AllGather、ReduceScatter、AllToAll 分别测了吗。
- 算法：ring、tree、hierarchical 是否匹配 message size 和拓扑。
...
- Artifact：benchmark 是否记录拓扑、rank mapping、版本、环境变量和网络 counters。
