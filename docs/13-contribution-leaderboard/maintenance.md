---
title: 榜单维护说明
description: 文章作者署名、贡献归属和每日排行榜维护方法
status: reviewed
owner: maintainers
license: CC-BY-4.0
updated: 2026-09-08
...
[查看每日排行榜](index.md)。榜单由仓库内的浏览量快照和文章作者信息自动生成，不需要手动维护排行行数或顺序。
...
无署名且无可追溯创建记录的文章会显示“作者未标注”，其浏览量仍进入文章排行榜，但不进入个人排行榜。

## 排行与统计范围

- 贡献者榜：先汇总该贡献者名下所有文章的累计浏览量，再取 Top 10；不是只汇总文章榜上的 10 篇。
- 文章榜：按文章累计浏览量取 Top 10，同时显示文章标题、全部作者和浏览量。
- 多作者文章完整计入每位作者，每位作者在同篇文章里只计一次。作者榜总和可能大于文章浏览量总和。
- 只纳入 `mkdocs.yml` 导航中的知识文章。首页、知识地图、主题 `index.md` 概览、模板、内部计划与榜单自身不参与统计。
- 未访问文章按 0 计入。并列时按贡献者名称或文章路径稳定排序；不足 10 项时显示实际数量。
...
本地生成只读取现有快照，不调用 GoatCounter API。`python -m scripts.update_leaderboard --help` 可查看命令帮助。生成的 `index.md` 会被覆盖，修改统计逻辑或页面说明请编辑 `scripts/update_leaderboard.py`，修改作者读取规则请编辑 `scripts/leaderboard_data.py`。
