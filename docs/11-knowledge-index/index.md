...
一篇文章可以署名多位实际贡献者。请在 front matter 中使用 `authors` 列表，并在不同文章中保持姓名或账号写法一致：
...
多人反复修改同一篇文章时，把希望在贡献榜显示的作者都列入这个列表。系统不会把每次 Git 修改者自动合并为作者；没有显式 `authors` 或单人 `author` 时，只按 Git 首次创建文件的作者归属，后续修改者不会自动入榜。`authors` 优先于 `author`，同一篇文章中重复的姓名只计一次。
...
- [基准实验报告模板：从问题设计到可复现实验](../99-templates/benchmark-report.md)
