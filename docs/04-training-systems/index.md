...
| 9 | [Collective 通信原语与通信量模型](collective-communication-primitives.md) | 理解 AllReduce、ReduceScatter、AllGather、AllToAll、Broadcast、process group、通信量模型、异步通信和 hang 排查。 |
| 10 | [Data Parallel 与梯度同步](data-parallel-gradient-sync.md) | 理解数据并行、AllReduce、gradient bucketing 和同步开销。 |
| 11 | [ZeRO 与 FSDP](zero-fsdp.md) | 学习参数、梯度和 optimizer state sharding 如何降低重复显存。 |
| 12 | [Tensor Parallel](tensor-parallel.md) | 学习把单层矩阵和 attention 计算切到多 GPU 的方法。 |
| 13 | [Sequence Parallel 与 Context Parallel](sequence-context-parallel.md) | 理解长序列训练中如何切分 token 序列或上下文，降低 activation 和 attention 压力。 |
| 14 | [Pipeline Parallel](pipeline-parallel.md) | 学习层间切分、micro-batch 流水和 pipeline bubble。 |
| 15 | [Expert Parallel 与 MoE 训练](expert-parallel-moe-training.md) | 处理专家路由、token dispatch、负载均衡和跨卡通信。 |
| 16 | [并行策略组合：3D/4D/5D Parallelism](hybrid-parallelism-composition.md) | 把 DP/FSDP、TP、PP、EP、SP/CP 放到同一个 rank topology 中理解。 |
| 17 | [Activation Checkpointing](activation-checkpointing.md) | 用重计算换显存，降低长上下文和大 batch 的 activation 压力。 |
| 18 | [混合精度训练](mixed-precision-training.md) | 理解 FP16、BF16、FP8、loss scaling 和数值稳定性。 |
| 19 | [训练稳定性与数值异常：NaN、Inf、Loss Spike 与梯度健康](training-stability-numerical-debugging.md) | 把 NaN/Inf、loss spike、grad norm、loss scale、bad batch、checkpoint rollback 和 stability guardrail 纳入训练系统。 |
| 20 | [通信与计算重叠](communication-computation-overlap.md) | 分析 backward、AllReduce/ReduceScatter、bucket 和 overlap 失败原因。 |
| 21 | [FLUX 通信重叠与 Kernel Fusion](flux-kernel-fusion.md) | 以 FLUX 为案例，理解如何把通信和计算细粒度融合来隐藏分布式通信。 |
| 22 | [Optimizer 与 Scheduler 系统成本](optimizer-scheduler-cost.md) | 研究 Adam/AdamW、fused optimizer、学习率调度和 optimizer state 成本。 |
...
分布式训练里的数据交换通常由 collective 通信完成。理解 AllReduce、ReduceScatter、AllGather、AllToAll、Broadcast、process group、通信量模型、异步语义和 hang 排查，是分析 DP/FSDP/TP/PP/EP 通信开销的公共基础。

详见：[Collective 通信原语与通信量模型](collective-communication-primitives.md)
