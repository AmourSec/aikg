...
1. 用 `AI 计算工作负载基础` 理解性能瓶颈来自哪里。
2. 用 `推理系统与服务优化` 理解在线服务的延迟、吞吐和调度。
3. 用 `训练系统与分布式计算` 理解多卡、多机训练的并行和通信。
4. 用 `Kernel、算子与编译优化` 理解单个算子如何被做快。
5. 用 `AI 加速器与计算架构` 理解硬件如何约束上层系统。
6. 用 `性能分析、Benchmark 与容量建模` 验证任何优化是否真实有效。
