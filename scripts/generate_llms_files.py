...
PRIORITY_DOCS = [
    ...
    "11-knowledge-index/skills-authoring-guide.md",
    ...
    "12-hardware-basics/gpu-architecture-basics.md",
    "12-hardware-basics/npu-basics.md",
    "12-hardware-basics/gpu-npu-comparison.md",
...
DESCRIPTIONS = {
    ...
    "02-ai-workloads/transformer.md": "Transformer 流程与原理面向新手解释 decoder-only 大语言模型的数据流，覆盖文本到 token、tokenizer 词表与切分规则、token id、模型权重中的 embedding table、position information 的数字表示方式、self-attention、Q/K/V 参数矩阵来源、attention weights、scaled dot-product attention 直觉、causal mask、multi-head attention、MLP、残差连接、归一化、Transformer block、hidden states、LM Head、logits、训练时的 next-token prediction、推理时的自回归生成、KV Cache 直觉、encoder-only/decoder-only/encoder-decoder 区别、Transformer 的并行性和系统成本来源。",
    "02-ai-workloads/training-primer.md": "训练过程与原理面向新手解释大语言模型训练的最小闭环，覆盖 next-token prediction、shifted labels、batch、step、epoch、forward、logits、cross entropy loss、有效 token loss 汇总、backward、autograd graph、梯度、optimizer step、learning rate、zero_grad、mini-batch 估计、validation/test、pretraining、finetuning、post-training、checkpoint、模型权重产物，以及训练和推理的区别。",
    ...
    "03-inference-systems/prefill-decode-disaggregation.md": "Prefill/Decode 分离部署用独立资源池和 KV 传输减少两阶段互相干扰，并解释异构下的 AFD / 异构下 AFD（Attention-FFN Disaggregation）如何进一步按 Attention 与 FFN/MLP 角色拆分资源池、通信路径和调度责任。",
    ...
    "03-inference-systems/rag-agent-workloads.md": "RAG 与 Agent 作为复合推理 workload，拆解检索、rerank、context packing、工具调用、多轮 LLM 调用、代码执行沙箱、沙箱快启、PVM/pre-warmed VM/pooled VM/persistent VM 池化、缓存、容量、可靠性和端到端 benchmark。",
    ...
    "05-kernels-compilers/index.md": "Kernel、算子与编译优化主题入口，覆盖算子语义、手写 Kernel、Tile 与 Schedule、图编译与代码生成、编译 IR 与 Lowering、跨算子融合、性能诊断、Attention、Triton、Triton 编译器、Triton AI 算子生成、TorchInductor、MLIR、TileLang、OpenTileIR/open tile IR、Persistent Kernel 和 MegaKernel。",
    ...
    "05-kernels-compilers/triton.md": "Triton Kernel 编程解释 blocked program 执行模型、Triton 编译器 pipeline、Triton AI算子生成 / Triton AI 算子生成闭环、JIT 生命周期、launch grid、program_id、block tensor、pointer arithmetic、mask、shape specialization、AI workload 模式、tiling、资源模型、数值与 dtype、fused softmax、matmul、autotune、PyTorch/Inductor 集成、debugging、benchmark、端到端验证和 profiler 方法。",
    ...
    "12-hardware-basics/index.md": "硬件基础章节入口，按通用硬件架构、昇腾平台与 CANN、硬件知识沉淀三组组织 GPU 架构、NPU 架构、GPU/NPU 对比、Ascend 型号、SocVersion、NpuArch、Ascend 910/950、CANN 软件栈和硬件适配 AI skill 样例。",
    ...
    "12-hardware-basics/npu-basics.md": "NPU 架构基础结合 Ascend/CANN 官方与准官方架构图解释 NPU 执行模型、AI Core、Cube、Vector、Scalar、MTE/DataCopy、Local Memory、Global Memory/HBM、UB、L1、L0A/L0B/L0C、Cube 计算单元、UB 存储结构、Tiling、TPipe/TQue、GlobalTensor/LocalTensor、SIMD/SIMT/RegBase、compiler、runtime、profiling、AI workload 映射和 NPU 实验最小记录清单。",
    ...
    "12-hardware-basics/ascend-npu-models.md": "Ascend 型号、SocVersion 与 NpuArch 解释产品系列、芯片型号、SocVersion、NpuArch、__NPU_ARCH__、archXX 的关系，并给出 Ascend 910B、910_93、950PR、950DT 的工程判断边界。",
    "12-hardware-basics/ascend-910-series.md": "Ascend 910 系列平台要点结合 CANN/asc-devkit 官方或准官方架构图解释 910B/910_93、DAV_2201、Atlas A2/A3、AIC/AIV 分离、Cube、Vector、Scalar、MTE/DataCopy、UB、L1、L0A/L0B/L0C、Fixpipe、分形格式、训练/推理/算子场景、平台能力查询、Vector Add 骨架和按架构选择 2201 路径的代码样例。",
    "12-hardware-basics/ascend-950-series.md": "Ascend 950 系列平台要点结合 CANN/asc-devkit 官方或准官方架构图解释 950PR、950DT、DAV_3510、SIMT、SIMD-Regbase、L0C->UB、UB->L1、SSBuffer、ND-DMA、FP8/MX/HiF8、Cube、UB、L0C、HCCL 通信、2201 到 3510 迁移差异、平台能力查询、MatMul epilogue 和 SIMT gather 代码骨架。",
    "12-hardware-basics/cann-stack.md": "Ascend/CANN 软件栈与开发入口解释 Toolkit、Runtime、Driver、torch_npu、图模式、Ascend C、profiling、simulator、模型推理优化和 AI 诊断证据收集。",
    "12-hardware-basics/ai-skills-sample.md": "硬件适配 AI Skills 样例解释普通文档、benchmark、ADR、failure case 与 skill 的区别，并给出 NPU 架构能力判断 skill 的组织方式。",
    ...
    "11-knowledge-index/skills-authoring-guide.md": "AI Skills 编写指南说明如何把重复工作流程沉淀为 AI 可执行 skill，覆盖适用边界、skills/ 放置位置、SKILL.md front matter 与正文结构、references/scripts/assets 组织、从真实工作记录到 checklist 再到 skill 的抽象流程、llms.txt 更新、质量检查清单、常见错误、推荐起步方向和最小提交流程。",
...
def build_llms_txt(paths: list[str], skills: list[str]) -> str:
    ...
    lines = [
        ...
        "3. For efficient computing, move to `硬件基础`, `推理系统与优化`, `Kernel、算子与编译优化`, `AI 加速器与计算架构`, and `性能分析、Benchmark 与容量建模`.",
