...
DESCRIPTIONS = {
    ...
    "04-training-systems/parameter-efficient-finetuning-lora-qlora.md": "参数高效微调解释 LoRA、QLoRA 与 Adapter 如何通过冻结基础模型、训练低秩 adapter 降低 gradients、optimizer states 和 checkpoint 成本，并分析 activation、FSDP/ZeRO、artifact registry、serving cache key、多 adapter 推理和 benchmark 方法。",
    "04-training-systems/post-training-workloads-sft-dpo-rlhf-grpo.md": "后训练工作负载解释 SFT、reward model、DPO、RLHF/PPO 与 GRPO 如何形成不同训练系统数据流，覆盖 chosen/rejected 偏好数据、reference logprob、online rollout、reward/verifier、policy update、rollout/training 分离部署、LoRA/QLoRA 组合和后训练 benchmark 方法。",
    ...
    "04-training-systems/evaluation-validation-checkpoint-selection.md": "Evaluation、Validation 与 Checkpoint Selection 解释训练中的 validation loss、eval dataset 版本治理、eval cadence、同步/异步评估、eval 资源池、eval mode、loss/generation eval、scoring、decoding 参数、checkpoint selection、early stopping、分布式 eval、eval overhead、eval report 和常见故障排查。",
    "04-training-systems/training-reproducibility-randomness-run-manifest.md": "训练可复现性、随机性与 Run Manifest 解释 seed/RNG state、data order、DataLoader worker seed、rank seed policy、deterministic algorithms、floating point nondeterminism、mixed precision、activation checkpointing、MoE、rank mapping、checkpoint/resume、eval/benchmark 可复现性和结构化 run manifest。",
    ...
    "06-accelerators-architecture/accelerator-architecture-tradeoffs.md": "架构取舍解释 GPU、NPU、TPU、ASIC、FPGA 的通用性与专用性、执行模型、存储层次、精度支持、软件栈、动态性、扩展方式、算子覆盖和 workload 匹配。",
    "06-accelerators-architecture/workload-mapping-compiler-runtime-interface.md": "Workload Mapping 解释模型、IR、算子、kernel、layout、tiling、fusion、compiler、runtime、dynamic shape、fallback、parallel mapping 和硬件有效吞吐之间的关系。",
    ...
    "07-cluster-infra/mixed-workload-multitenancy-isolation.md": "混合集群与多租户隔离解释训练、在线推理、Notebook、离线批处理、数据预处理、benchmark、系统任务共存时的 workload classification、queue、quota、priority、preemption、node pool、GPU 共享、存储网络隔离、准入控制、资源碎片和成本归因。",
    "07-cluster-infra/resource-utilization-fragmentation-capacity.md": "资源利用率、碎片与容量治理解释 GPU 分配率、驻留率、活跃率、有效吞吐、pending reason、queue wait、fragmentation、largest contiguous allocation、公平性、SLA、成本归因、能效、DCGM、Kueue/Slurm 指标、dashboard、告警和容量规划。",
    "07-cluster-infra/node-lifecycle-health-maintenance.md": "节点生命周期与集群运维解释 AI 计算节点从资产登记、物理验收、软件基线、burn-in、入池、健康检查、Kubernetes/Slurm 状态、cordon/drain、维护验证、升级、配置漂移、自动扩缩容、本地缓存清理到退役下线的流程。",
    "08-benchmark-capacity/index.md": "性能分析、Benchmark 与容量建模主题入口。",
    "08-benchmark-capacity/performance-analysis-benchmark-methodology.md": "性能分析与 Benchmark 方法论解释 benchmark、profiling、monitoring 的区别，如何定义决策问题、latency/throughput/efficiency/cost/energy 指标、设计 workload、记录环境、处理 warmup、做样本统计、A/B、ablation、profiler 证据、瓶颈定位、回归检测和容量建模输入。",
    "08-benchmark-capacity/inference-capacity-modeling.md": "推理容量建模解释如何用 QPS、concurrency、TTFT、TPOT、E2E latency、goodput at SLA、输入/输出长度分布、prefill/decode、KV Cache token occupancy、batching、headroom、autoscaling、冷启动、路由、多模型和生产校准推导 GPU 副本数。",
    "08-benchmark-capacity/training-capacity-scaling-efficiency.md": "训练容量建模解释如何用 total tokens、global batch、micro batch、gradient accumulation、step time、tokens/s、tokens/s/GPU、MFU、strong/weak scaling efficiency、DP/TP/PP/EP/FSDP/ZeRO、checkpoint、eval、failure/restart、queue wait、成本和能效推导训练总时间与 GPU 需求。",
    "08-benchmark-capacity/profiler-toolchain-bottleneck-analysis.md": "Profiler 工具链与瓶颈定位解释如何用应用指标、PyTorch Profiler、Nsight Systems、Nsight Compute、DCGM、perf/eBPF、NVTX 标注、分布式 trace、kernel metrics、CPU/网络/存储信号和前后对比 benchmark 建立 AI 系统性能瓶颈证据链。",
    "08-benchmark-capacity/roofline-analysis-compute-bandwidth.md": "Roofline 分析解释如何用 FLOPs、bytes moved、arithmetic intensity、ridge point、peak compute、memory bandwidth、Nsight/profiler 指标、AI 算子形态、推理 prefill/decode、训练 forward/backward/optimizer、communication roofline 和端到端 benchmark 判断瓶颈上限与优化方向。",
    "08-benchmark-capacity/queueing-model-tail-latency.md": "排队模型与尾延迟解释如何用 Little's Law、QPS、concurrency、service time、utilization、queueing latency、Kingman/VUT 直觉、batching、head-of-line blocking、goodput at SLA、load shedding、deadline、retry budget、open-loop benchmark、请求长度分布和队列指标分析 AI 推理服务的 p95/p99 与容量边界。",
    "08-benchmark-capacity/energy-power-thermal-benchmark.md": "能效、功耗与热限制解释如何定义 GPU-only、node-level、rack/facility-level 测量边界，采集 power draw、energy、clocks、temperature、throttle reason、DCGM/NVML/nvidia-smi telemetry，并用 warmup、steady-state window、power cap sweep、joules/token、tokens/joule、energy to target、goodput at SLA、thermal headroom 和 power-aware scheduling 分析 AI 训练与推理系统能效。",
    "08-benchmark-capacity/benchmark-workload-design-trace-replay.md": "Benchmark 负载设计与 Trace Replay 解释如何把决策问题转成 workload spec，设计 synthetic workload、sampled workload、trace replay、input/output token 分布、request mix、arrival process、open-loop/closed-loop、coordinated omission 防护、cache 状态、RAG/Agent 负载、训练数据路径、warmup、measurement window、load generator 校验和 benchmark 报告 caveats。",
    "08-benchmark-capacity/ab-testing-ablation-regression-detection.md": "A/B 对比、消融实验与性能回归检测解释如何用 A/A test、noise floor、A/B comparison、ablation、primary metrics、guardrail metrics、explanatory metrics、paired runs、randomization、effect size、variance、baseline management、CI smoke benchmark、nightly benchmark、release gate、production canary、shadow traffic 和 profiler evidence 防止 AI 系统性能回归。",
    "08-benchmark-capacity/cost-model-unit-economics.md": "成本模型与单位经济性解释如何定义 GPU-only、node-level、service-level、workflow-level 成本边界，计算 cost/request、cost/input token、cost/output token、cost/training token、cost/successful run、cost to target quality，并把 GPU hour、CPU、内存、存储、网络、能源、失败重试、headroom、缓存、共享平台成本、标签归因、多租户和研发速度纳入 AI 系统单位经济性。",
    "08-benchmark-capacity/benchmark-data-governance-run-records.md": "Benchmark 数据治理与实验记录解释如何用 run manifest、run id、raw per-request/per-step/system metrics、summary schema、environment metadata、artifact management、lineage、timestamp alignment、privacy redaction、retention、baseline registry、dashboard/report separation、CI regression records 和 AI-readable Markdown/YAML/JSON 让 AI benchmark 结论可复现、可审查、可比较和可检索。",
