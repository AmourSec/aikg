...
DESCRIPTIONS = {
    ...
    "05-kernels-compilers/index.md": "Kernel、算子与编译优化主题入口，覆盖算子语义、手写 Kernel、Tile 与 Schedule、图编译与代码生成、编译 IR 与 Lowering、跨算子融合、性能诊断、Attention、Triton、TorchInductor、MLIR、TileLang 和 MegaKernel。",
    ...
    "05-kernels-compilers/tilelang.md": "TileLang：面向 AI Kernel 的 Tile 编程模型解释 tile-oriented DSL 在 AI kernel 中的位置，覆盖 tile、dataflow、schedule、layout、thread binding、pipeline、tensorization、autotuning、与 Triton/TVM/MLIR 的关系、适用场景、benchmark 方法和工程检查清单。",
    "05-kernels-compilers/megakernel-persistent-automatic-generation.md": "MegaKernel、Persistent Kernel 与自动生成解释普通 fusion、CUDA Graph、persistent kernel、MegaKernel 的区别，覆盖 LLM 推理为何关注 persistent kernel、Triton MegaKernel、Ascend C MegaKernel-style 实现、自动生成 pipeline、任务图/Event/IR-based 方向、收益来源、风险、benchmark 方法和设计决策表。",
